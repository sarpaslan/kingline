	...
	public static class WhatUsesThis
	{
		...
		static Dictionary<string, List<string>> CleanBuild()
		{
			try
			{
				...
				var allAssets = AssetDatabase.FindAssets("").Select(x => AssetDatabase.GUIDToAssetPath(x)).Distinct()
					.ToArray();
				...
				foreach (var asset in allAssets)
				{
					...
					if (i % 100 == 0 && EditorUtility.DisplayCancelableProgressBar("WhatUsesThis",
						    $"Getting Dependancies [{i}/{allAssets.Length}]", i / (float)allAssets.Length))
		...
		private static void FindParentAssets()
		{
			...
			Debug.Log(
				$"<color=#5C93B9>Search complete, found <b>{iCount}</b> result{(iCount == 1 ? "" : "s")}</color>");
		}
